...
A. $-F(x_0)$  
B. $F(x_0)$  
C. $\displaystyle\frac{a}{2}-\int_0^{x_0}f(x)\,dx$  
...
A. $\displaystyle F(x)=\begin{cases}\frac{1}{3}x^3+x,&x\le0,\\ \sin x+1,&x>0\end{cases}$  
B. $\displaystyle F(x)=\begin{cases}\frac{1}{3}x^3+x+1,&x\le0,\\ \sin x+2,&x>0\end{cases}$  
C. $\displaystyle F(x)=\begin{cases}\frac{1}{3}x^3+x+1,&x\le0,\\ \sin x,&x>0\end{cases}$  
D. $\displaystyle F(x)=\begin{cases}\frac{1}{3}x^3+x,&x\le0,\\ \sin x,&x>0\end{cases}$
...
**(8)** 设 $f(x)$ 在 $[0,1]$ 上连续，$f(x)>0,f'(x)<0,f''(x)>0$，记 $\displaystyle M=\int_0^1f(x)\,dx,\ N=f(1),\ P=\frac{1}{2}[f(0)+f(1)]$，则（　）.
...
**(1)** 设 $F(x)$ 是 $f(x)$ 的一个原函数，$F\left(\dfrac{\pi}{4}\right)=0$，当 $\dfrac{\pi}{4}<x<\dfrac{\pi}{2}$ 时，$F(x)>0$，$\displaystyle F(x)f(x)=\frac{\ln(\tan x)}{\sin x\cos x}$，则 $f(x)=\underline{\qquad}$.
...
**(8)** 设 $\displaystyle\int_1^{+\infty}\frac{dx}{x(a+x^3)}=\frac{1}{3}\ln2\ (a>0)$，则 $a=\underline{\qquad}$.

**(9)** 函数 $\displaystyle y=\frac{x^2}{\sqrt{1-x^2}}$ 在 $\left[\dfrac{1}{2},\dfrac{\sqrt3}{2}\right]$ 上的平均值为 $\underline{\qquad}$.

**(10)** 设 $\displaystyle f(x)=\int_0^{a-x}e^{t(2a-t)}\,dt\ (a>0),\ x\in[0,a]$，则曲线 $y=f(x)$ 与两坐标轴所围成图形的面积为 $\underline{\qquad}$.

**(11)** 曲线 $\displaystyle y=\frac{\sqrt{x}}{1+x^2}$ 绕 $x$ 轴旋转一周所得的旋转体，将它在 $x=0$ 与 $x=\xi\ (\xi>0)$ 之间部分的体积记为 $V(\xi)$，且 $\displaystyle V(a)=\frac{1}{2}\lim_{\xi\to+\infty}V(\xi)$，则 $a=\underline{\qquad}$.

**(12)** 曲线 $\displaystyle r=a\sin^3\frac{\theta}{3}\ (a>0,0\le\theta\le3\pi)$ 的弧长 $s=\underline{\qquad}$.
...
$$
\int_0^af(x)\,dx>af\left(\frac{a}{2}\right).
$$
...
**(15)** 设曲线 $y=\sin x\ \left(0\le x\le\dfrac{\pi}{2}\right)$、直线 $y=k\ (0\le k\le1)$ 与 $x=0$ 所围面积为 $S_1$，$y=\sin x\ \left(0\le x\le\dfrac{\pi}{2}\right)$、$y=k$ 与 $x=\dfrac{\pi}{2}$ 所围面积为 $S_2$，求 $S=S_1+S_2$ 的最小值.
...
**(2)** 设 $f(x)$ 在 $\left(0,\dfrac{\pi}{2}\right)$ 内可导，$f(x)>0,\displaystyle\lim_{x\to0^+}f(x)=1$，且
...
**(5)** 曲线 $\displaystyle\theta=\frac{1}{2}\left(r+\frac{1}{r}\right)$ 在区间 $r\in[1,3]$ 上的弧长为 $\underline{\qquad}$.
...
**(8)** 设 $f(2)=\dfrac{1}{2},f'(2)=0$，且 $\displaystyle\int_0^2f(x)\,dx=1$，则
...
**(10)** 已知 $\displaystyle\int_0^{+\infty}\frac{\sin x}{x}\,dx=\frac{\pi}{2}$，则
...
**(13)** $\displaystyle\lim_{x\to0^+}\frac1{\ln\frac{1}{x}}\int_x^{+\infty}\frac{e^{-t}}t\,dt=\underline{\qquad}$.
...
$$
\int_0^x f(t-x)\,dt=x+\frac{1}{2}x^2+\ln|1-x|,
$$
...
**(4)** 计算 $\displaystyle I=\int\frac{e^{-\sin x}\cdot\sin2x}{\sin^4\left(\frac{\pi}{4}-\frac{x}{2}\right)}\,dx$.
...
$$
\lim_{x\to0}\frac{\dfrac{1}{2}\displaystyle\int_0^2x\sqrt{4-x^2u^2}\,du-2x}{\sqrt{1+2x^3}-1}.
$$

**(8)** 设 $f(x)$ 在 $(-\infty,0]$ 上连续，且满足

$$
\int_0^x tf(t^2-x^2)\,dt=\frac{x^2}{1+x^2}-\frac{1}{2}\ln(1+x^2),
$$
...
$$
\lim_{n\to\infty}\left(
\frac{2^{1/n}}{n+1}+\frac{2^{2/n}}{n+\frac{1}{2}}+\cdots+\frac{2^{n/n}}{n+\frac{1}{n}}
\right).
$$

**(15)** 求极限 $\displaystyle\lim_{n\to\infty}\frac{1}{n}\sqrt[n]{n(n+1)(n+2)\cdots(2n-1)}$.
...
**(32)** 设 $f(x)=x^n\sqrt{1-x^2},x\in[0,1]$ 与 $y=0$ 所围平面区域的面积为 $S_n$，$g(x)=\sin^{n/2}x,x\in\left[0,\dfrac{\pi}{2}\right]$ 与 $y=0$ 所围平面区域绕 $x$ 轴旋转一周所得体积为 $V_n\ (n=1,2,\cdots)$，求极限 $\displaystyle\lim_{n\to\infty}\frac{\pi S_n}{V_n}$.
...
**(37)** 设心形线 $r=4(1+\cos\theta)$ 与 $\theta=0,\theta=\dfrac{\pi}{2}$ 所围图形为 $D$，求 $D$ 绕极轴旋转一周所得旋转体的体积.
...
**(40)** 设有一质量为 $m$ 的均匀杆 $AB$，$A$、$B$ 的坐标分别为 $\left(-\dfrac{l}{2},0\right)$ 与 $\left(\dfrac{l}{2},0\right)$，单位质量的质点 $C$ 的坐标为 $(0,a)\ (a>0)$.
...
**(I)** 当 $x\in[0,1]$ 时，有 $\displaystyle\int_0^x f(t)\,dt\ge\frac{1}{2}[xf(x)+x]$；

**(II)** $\displaystyle\int_0^1\left(\frac{2}{3}-x\right)f(x)\,dx\ge\frac{1}{6}$.
...
+\frac{1}{2}\int_a^b(x-a)(x-b)f''(x)\,dx;
$$

**(II)** 记 $\displaystyle M=\max_{x\in[a,b]}\{|f''(x)|\}$，证明：

$$
\left|\int_a^b f(x)\,dx-\frac{1}{2}(b-a)[f(a)+f(b)]\right|
\le\frac{(b-a)^3}{12}M.
$$

**(4)** 设 $f(x)$ 在 $[0,+\infty)$ 上有连续的二阶导数，且 $|f''(x)|\le1$.

**(I)** 证明：$\displaystyle\left|\int_0^1f(x)\,dx-f\left(\frac{1}{2}\right)\right|\le\frac1{24}$.

**(II)** 若对任意的 $x\in[0,+\infty)$，有 $f(x)=f(x+1)$，$n$ 为正整数. 证明：

$$
\left|\int_0^n f(x)\,dx\right|\le n\left[\frac{1}{6}+|f(0)|\right].
$$
